...
public class OSC : MonoBehaviour
{
    ...
    //        try
    //        {
    //            //IP = JsonReader.instance.data.fileData[0].IP_Address_osc;
    ...
    public void StartIP()
    {
        try
        {
            // Get the current IP from PlayerPrefs if available
            IP = PlayerPrefs.GetString("ip", IP); // fallback to previous IP if key doesn't exist

            // Close any existing UDP connection before opening a new one
            if (OscPacketIO != null && OscPacketIO.IsOpen())
            {
                OscPacketIO.Close();
                OscPacketIO = null;
                Debug.Log("Existing OSC connection closed.");
            }

            // Initialize new UDP connection
            OscPacketIO = new UDPPacketIO(IP, outPort, inPort);

            // Reset or initialize required fields
            AddressTable = new Hashtable();
            messagesReceived = new ArrayList();
            buffer = new byte[1000];

            // Stop previous read thread if running
            if (ReadThread != null && ReadThread.IsAlive)
            {
                ReaderRunning = false;
                ReadThread.Join(); // safely wait for thread to stop
            }
            ...
            Debug.Log("OSC listener started on IP: " + IP + " OutPort: " + outPort + " InPort: " + inPort);
        }
        catch (Exception e)
        {
            Debug.LogError("Failed to start OSC: " + e);
        }
    }
